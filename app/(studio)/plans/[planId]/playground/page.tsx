...
import {
  MicroPracticeLab,
  type QuizResults,
} from "./components/MicroPracticeLab";
import { FeynmanCheck } from "./components/FeynmanCheck";
import {
  OmniWorkspace,
  type EnvMode as OmniEnvMode,
} from "./components/OmniWorkspace";
import {
  VerificationEngine,
  type ArtifactVerifiedEvent,
} from "./components/VerificationEngine";
...
const getInitialCode = (
  task: { ai_generated_examples?: unknown[] } | undefined,
) => {
...
function resolveExecutionDescriptor(
  ...
  if (direct && typeof direct === "object")
    return direct as ExecutionDescriptor;
  const verification = task.verification;
  if (
    verification &&
    typeof verification === "object" &&
    verification.execution_descriptor &&
    typeof verification.execution_descriptor === "object"
  ) {
  ...
  if (
    env &&
    typeof env === "object" &&
    env.execution_descriptor &&
    typeof env.execution_descriptor === "object"
  ) {
...
function PlaygroundFlow() {
  ...
  const [feynmanConversationId, setFeynmanConversationId] = useState<
    string | null
  >(null);
  ...
  const changeStep = (newIndex: number) => {
    ...
    setStepDurations((prev) => ({
      ...prev,
      [activeStepIndex]: (prev[activeStepIndex] || 0) + elapsed,
    }));
  ...
  const [challengeVerification, setChallengeVerification] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const executionDescriptor = useMemo(
    () => resolveExecutionDescriptor(activeTask),
    [activeTask],
  );
  ...
  const safeRefetchPlan = useCallback(() => {
    if (planRefetchInFlightRef.current) return;
    planRefetchInFlightRef.current = true;
    refetchPlan().finally(() => {
      planRefetchInFlightRef.current = false;
    });
  }, [refetchPlan]);
  ...
  const [verificationResult, setVerificationResult] =
    useState<ArtifactVerifiedEvent | null>(null);
  const [executionDiagnostics, setExecutionDiagnostics] =
    useState<ExecutionDiagnostics | null>(null);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("artifact_verified", handleArtifactVerified);
  }, [activeTask?.id]);
  ...
  useEffect(() => {
    ...
    const isComplete =
      Array.isArray(v.problem_set) && v.problem_set.length >= 2;
    ...
    planningApi
      .generateChallenge(activeTask.id)
      .then((res) => {
        if (res.verification) {
          ...
          challengePollRef.current = setInterval(() => safeRefetchPlan(), 5000);
  ...
  }, [currentStepId, activeTask, challengeVerification, safeRefetchPlan]);
  ...
  const starterCodePollRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    planningApi
      .generateStarterCode(activeTask.id)
      .then((res) => {
        if (res.starter_code) {
          setStarterCode(res.starter_code);
          setStarterCodeLoading(false);
        } else {
          starterCodePollRef.current = setInterval(
            () => safeRefetchPlan(),
            5000,
          );
        }
  ...
  }, [currentStepId, activeTask, starterCode, safeRefetchPlan]);
  ...
  const isLessonThin = (task: typeof activeTask): boolean => {
    ...
    if (
      ...
      (conceptBlock.content || "")
        .trim()
        .toLowerCase()
        .startsWith(description.slice(0, 60))
    )
      return true;
    return false;
  };
  ...
  const pendingLessonGenerationRef = useRef<{
    taskId: string;
  ...
  useEffect(() => {
    ...
    const stored = window.localStorage.getItem("playgroundSceneMode");
    if (stored === "basic" || stored === "extended") {
      setSceneMode(stored);
    }
  }, []);

  const startLessonPolling = () => {
    ...
    lessonPollRef.current = setInterval(() => safeRefetchPlan(), 5000);
  ...
  const lessonSnapshotForTask = (task: typeof activeTask): string => {
    if (!task) return "none";
    const blocks = task.lesson_blocks ?? [];
    const head = blocks
      .slice(0, 3)
      .map((b) => `${b.type || "unknown"}:${b.title || ""}`)
      .join("|");
    return `${task.lesson_generated_at || "null"}::${blocks.length}::${head}`;
  };
  const regenerationLabel = (() => {
    const pending = pendingLessonGenerationRef.current;
    if (!lessonLoading || !pending) return null;
    const shortTask = pending.taskId.slice(0, 8);
    return `Regenerating ${shortTask}`;
  })();

  // Stop lesson polling as soon as the lesson blocks arrive and are no longer thin
  useEffect(() => {
    if (!lessonPollRef.current || !activeTask) return;

    const pending = pendingLessonGenerationRef.current;
    if (!pending) return;
    if (pending.taskId !== activeTask.id) return;

    const currentGeneratedAt = activeTask.lesson_generated_at ?? null;
    const currentSnapshot = lessonSnapshotForTask(activeTask);
    const hasFreshVersion =
      currentGeneratedAt !== pending.previousGeneratedAt ||
      currentSnapshot !== pending.previousSnapshot;

    if (!hasFreshVersion || isLessonThin(activeTask)) return;
    ...
    safeRefetchPlan();
    setLessonLoading(false);
  }, [activeTask, planId, queryClient, safeRefetchPlan]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleProofSubmit = async (
    ...
    try {
      ...
      const payload =
        type === "file" && content instanceof File
          ? (() => {
              ...
              formData.append(
                "metadata",
                JSON.stringify({ ...metadata, file_name: content.name }),
              );
      ...
      const proofResponse = await planningApi.submitTaskProof(
        activeTask.id,
        payload,
      );
      const executionReport = proofResponse.execution_report as
        ...
        | undefined;
      const diagnostics = proofResponse.execution_diagnostics as
        | ExecutionDiagnostics
        | undefined;
        ...
        if (executionReport.passed) {
          ...
          telemetry.toastError(
            executionReport.summary ||
              "Auto-tests failed. Review your code and resubmit.",
          );
      ...
      verifyTimeoutRef.current = setTimeout(() => {
        setIsVerifying(false);
        telemetry.toastSuccess(
          "Submission received. Check your portfolio for results.",
        );
  ...
  const buildMentorContext = (): string => {
    ...
    if (quizResults) {
      ...
      if (quizResults.weakTopics.length)
        parts.push(
          `Struggling with: ${quizResults.weakTopics.slice(0, 3).join(", ")}`,
        );
    }
    const unhelpful = Object.entries(blockFeedback)
      ...
      .map(([k]) => k);
    if (unhelpful.length)
      parts.push(`Found confusing: lesson blocks ${unhelpful.join(", ")}`);
    if (feynmanGaps.length)
      parts.push(`Feynman gaps: ${feynmanGaps.slice(0, 3).join(", ")}`);
    if (currentStepId === "omni") {
      const envReqs = activeTask?.environment_requirements as
        | Record<string, unknown>
        | undefined;
  ...
  const handleMentorReviewRequest = (content: string) => {
    const criteria =
      activeTask?.check_in_question || activeTask?.description || "";
    handleMentorSend(
  ...
  const envReqs = activeTask?.environment_requirements as
    | Record<string, unknown>
    | undefined;
  const recommendedEnvRaw = envReqs?.recommended_environment as
    | PlaygroundEnvMode
    | undefined;
  const subjectCategory = envReqs?.subject_category as string | undefined;
  const recommendedEnv = recommendedEnvForSurface(
    executionDescriptor,
    recommendedEnvRaw,
  );
  const defaultLanguage = inferDefaultLanguage(activeTask, recommendedEnv);
  const visibleEnvModes = computeVisibleEnvs(
    subjectCategory,
    recommendedEnv,
    executionDescriptor,
  );
  ...
  const missionCriteria = (() => {
    const fromChallenge = parseCriteriaList(
      challengeVerification?.acceptance_criteria,
    )
      .map(cleanMissionText)
      .filter(Boolean);
    if (fromChallenge.length) return fromChallenge;
    const fromTaskVerification = parseCriteriaList(
      activeTask?.verification?.criteria,
    )
      .map(cleanMissionText)
      .filter(Boolean);
    if (fromTaskVerification.length) return fromTaskVerification;
    const fromCheckin = parseCriteriaList(activeTask?.check_in_question)
      .map(cleanMissionText)
      .filter(Boolean);
    return fromCheckin.length
      ? fromCheckin
      : ["Demonstrate a complete, testable solution for this task."];
  })();
  ...
  const onRegenerateLesson = () => {
    ...
    const resolvedTaskId =
      (urlTaskId && tasks.some((t) => t.id === urlTaskId) ? urlTaskId : null) ||
      activeTask?.id;
    if (!resolvedTaskId) return;

    const targetTask =
      tasks.find((t) => t.id === resolvedTaskId) ?? activeTask ?? null;
  ...
  const toggleSceneMode = () => {
    setSceneMode((prev) => {
      const next: SceneMode = prev === "extended" ? "basic" : "extended";
      if (typeof window !== "undefined") {
        window.localStorage.setItem("playgroundSceneMode", next);
      }
      if (activeTask?.id) {
    ...
    if (currentStepId === "ingest" && lessonLoading) {
      return {
        ...
        metadata: {
          ...
          detail:
            "Horizon is tailoring your Learn tab with examples and guidance.",
        },
      };
    }
    if (currentStepId === "omni" && starterCodeLoading) {
      return {
        ...
        metadata: {
          ...
          detail:
            "Starter code and scaffolding are being prepared for this task.",
        },
      };
    }
    if (currentStepId === "verify" && challengeLoading) {
      return {
        ...
        metadata: {
          ...
          detail:
            "Acceptance criteria and challenge checks are being generated.",
        },
      };
    }
    return null;
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] min-h-0 flex-col gap-4 overflow-hidden bg-slate-50 p-4 lg:p-6">
          ...
          <div
            className={
              isExtendedSceneTask
                ...
                : "flex min-h-0 flex-col gap-4 md:col-span-8"
            }
          >
            {activeTask && !isExtendedSceneTask && (
              <TaskOrientationBanner
                taskId={activeTask.id}
                surfaceRationale={
                  activeTask.surface_rationale ??
                  `Complete the ${activeTask.title} task using the steps below.`
                }
                steps={steps}
              ...
              <TaskMissionBrief
                ...
                lessonBlockTitles={
                  activeTask?.lesson_blocks
                    ?.filter(
                      (b) => b.type === "concept" || b.type === "objective",
                    )
                    .slice(0, 2)
                    .map((b) => b.title)
            ...
            {surfaceRuntimeState ? (
              <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm text-blue-900">
                <p className="font-semibold">
                  {String(
                    surfaceRuntimeState.metadata?.title ||
                      "Preparing your workspace",
                  )}
                </p>
                <p className="text-xs text-blue-700">
                  {String(
                    surfaceRuntimeState.metadata?.detail ||
              ...
              <div className="min-h-0 flex-1 animate-in fill-mode-both fade-in slide-in-from-bottom-4 duration-500">
                {taskSurfaceType === "flashcard_session" ? (
                  <FlashcardSessionSurface
                    ...
                    onComplete={() => {
                      setCompletedSteps(
                        (prev) => new Set([...prev, activeStepIndex]),
                      );
                      changeStep(activeStepIndex + 1);
                    }}
                  />
            ...
            {currentStepId === "scenario" &&
              taskSurfaceType === "simulation_scenario" && (
                <div className="min-h-0 flex-1 overflow-y-auto pr-2 custom-scrollbar animate-in fill-mode-both fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex min-h-0 flex-1 animate-in flex-col gap-3 fill-mode-both fade-in slide-in-from-bottom-4 duration-500">
                {quizFailed && !quizBannerDismissed && (
                  <div className="shrink-0 flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3">
                    <span className="text-amber-600 text-sm font-medium flex-1">
                      ⚠ You scored {quizResults!.correct}/{quizResults!.total}{" "}
                ...
                <OmniWorkspace
                  ...
                  onDiagramExport={(file) => {
                    ...
                    telemetry.toastSuccess(
                      "Diagram attached for verification.",
                    );
                  }}
                  onRequestMentorReview={handleMentorReviewRequest}
                  onExecutionEvent={(event) => {
            ...
            {currentStepId === "verify" && (
              <div className="min-h-0 flex-1 animate-in fill-mode-both fade-in slide-in-from-bottom-4 duration-500">
                <VerificationEngine
                  ...
                  problemSet={
                    (challengeVerification?.problem_set as Array<
                      Record<string, unknown>
                    >) || undefined
                  }
                  hiddenTestIntent={
      ...
      <button
        ...
        className="fixed bottom-5 right-5 z-50 rounded-full border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-lg hover:bg-slate-50"
        title="Toggle lesson generation mode"
      >
