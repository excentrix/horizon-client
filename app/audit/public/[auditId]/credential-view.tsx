          ...
          <main className="flex flex-1 flex-col py-10">
            ...
            {v?.improvement_note && (
              <motion.section custom={5} variants={fadeUp} initial="hidden" animate="show" className="mt-10">
                <p className="caseline mb-3 text-center uppercase tracking-[0.22em]">How to strengthen this</p>
                <div className="rounded-2xl border border-border bg-card p-5">
                  <ImprovementNoteCard note={v.improvement_note} />
                </div>
              </motion.section>
            )}
            ...
            {/* ── The full record ── */}
            {!!v?.transcript?.length && (
              <motion.section custom={6} variants={fadeUp} initial="hidden" animate="show" className="mt-10">
                <TranscriptPanel
                  turns={v.transcript.map((turn) => ({
                    question: turn.question,
                    answer: turn.answer,
                    area: turn.area,
                    gradingStatus: turn.grading_status,
